...
-(void)publishWish {
    ...
    /**上传数据*/
    BmobObject *wishObj = [BmobObject objectWithClassName:@"WishList"];
    [wishObj setObject:_textView.text forKey:@"link"];
    [wishObj setObject:_tv1.text forKey:@"content"];
    [wishObj setObject:[BmobUser currentUser] forKey:@"wishUser"];
    ...
    if (_headerUrl) {
        ...
        BmobFile *file = [[BmobFile alloc]initWithFileName:@"picture.jpg" withFileData:imgData];
        ...
        [file saveInBackgroundByDataSharding:^(BOOL isSuccessful, NSError *error) {
            ...
            if (isSuccessful) {
                ...
                [wishObj saveInBackgroundWithResultBlock:^(BOOL isSuccessful, NSError *error) {
                    ...
                    if (isSuccessful) {
                        ...
                        if ([db open]) {
                            ...
                            BOOL result = [db executeUpdate:@"CREATE TABLE IF NOT EXISTS Wish_List (id integer PRIMARY KEY,link text,content text, fileUrl blob, nickName text, headUrl text, objectId text);"];
                            ...
                            if (result) {
                                ...
                                [q getObjectInBackgroundWithId:[BmobUser currentUser].objectId block:^(BmobObject *object, NSError *error) {
                                    
                                   BOOL is = [db executeUpdate:@"INSERT INTO Wish_List (link,content,fileUrl,nickName,headUrl) VALUES (?,?,?,?,?);",_textView.text,_tv1.text,imgData,[object objectForKey:@"nickName"],[object objectForKey:@"userHeader"],wishObj.objectId];
                ...
                NSLog(@"文件上传失败---%@", error);
                [HUDUtils setupErrorWithStatus:@"图片上传失败" WithDelay:1.5f completion:nil];
            }
