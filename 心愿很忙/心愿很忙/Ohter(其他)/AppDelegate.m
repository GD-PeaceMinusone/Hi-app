...
-(void)setupRongyun {
    ...
    [[RCIM sharedRCIM] setGlobalNavigationBarTintColor:HiBlueColor];
...
-(void)getUserInfoWithUserId:(NSString *)userId completion:(void (^)(RCUserInfo *))completion {
    ...
    [query getObjectInBackgroundWithId:[BmobUser currentUser].objectId block:^(BmobObject *object, NSError *error) {
        ...
        self.meUserId = [object objectForKey:@"userId"];
        
        if ([userId isEqualToString:self.meUserId]) {
            
            info.userId = userId;
            info.name = [object objectForKey:@"nickName"];
            info.portraitUri = [object objectForKey:@"userHeader"];
